...
\textbf{Application version} & 2.2.0 \\[5pt]
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt]
  ...
  \item Type priority: the \texttt{.docx}-derived PDF (\texttt{\_docx.pdf}) takes
        precedence over the native \texttt{.pdf} of the same document and language.
...
% ======================================================================
\section{Document type priority}
% ======================================================================
In production, the same document may be downloaded in two formats: the native
\texttt{.pdf} and the version converted from \texttt{.docx}, which the source
system delivers as \texttt{\_docx.pdf} (the \texttt{.docx} extension becomes a
\texttt{\_docx} suffix). Up to four files may therefore arrive: two languages
$\times$ two types.

The priority rule is applied \textbf{after} the language filter: for the same
document and language, the \texttt{.docx}-derived copy (\texttt{\_docx.pdf}) takes
precedence over the native \texttt{.pdf}, which is closed automatically. The rule
is symmetric, so it works regardless of which type is downloaded first.
...
\bottomrule
\endfoot
Only the native \texttt{.pdf} & The native one opens. \\
Only the \texttt{\_docx.pdf} & The docx-derived one opens. \\
Both types of the same document and language & The \texttt{\_docx.pdf} remains; the native one closes. \\
\bottomrule
\end{xltabular}

The browser copy suffix (\texttt{(1)}, \texttt{(2)}, \ldots) is stripped uniformly
across every file-identity key. As a result, a more recent copy correctly replaces
the open one and language pairing keeps working even when one of the files is a
copy (e.g. \texttt{\_docx (1).pdf}).
...
\subsection{Lightweight single-file build}
To distribute the application as a single executable of about 1.8~MB, the app and
the WebView2 DLLs (including the native \texttt{WebView2Loader.dll}) are bundled
inside the \texttt{.exe}, \textbf{without} embedding .NET. The target machine still
needs the \emph{.NET~8 Desktop Runtime} and the \emph{WebView2 Runtime}. The
\texttt{publish.bat} script runs this same command.

\begin{lstlisting}[caption={Lightweight build (single file, no embedded .NET).}]
...
The executable is produced at \ruta{publish-light\bs PdfAutoViewer.exe}; the
adjacent \texttt{.xml} and \texttt{.pdb} files are not needed at runtime. For a
fully standalone build that does \textbf{not} require .NET on the target (bundles
the runtime, $\sim$80~MB), use \texttt{-{}-self-contained true}.
...
\bottomrule
\endfoot
2.2.0 & June 2026 & Type priority: the \texttt{.docx}-derived PDF (\texttt{\_docx.pdf}) takes precedence over the native \texttt{.pdf} after the language filter; uniform handling of the \texttt{(n)} copy suffix across every file-identity key; \texttt{publish.bat} produces the lightweight single-file build. \\
